<x-layouts.app title="Add Section">
    <div class="flex h-full w-full flex-1 flex-col gap-4 rounded-xl">
        ...
        <div class="relative  py-6 px-4 overflow-auto rounded-xl border border-neutral-200 dark:border-neutral-700">
            <h2 class="text-lg font-bold mb-4">Add Content (For - {{ $section->section_name }})</h2>
            @session('error')
            ...
            <form method="post" enctype="multipart/form-data">
                @csrf
                <div class="mb-5">
                    <flux:input label="Title" name="title" required autofocus type="text" placeholder="Enter Title"
                        class="w-full mb-4" />
                    ...
                    <flux:input label="Link" name="link" required autofocus type="url" placeholder="Enter Link"
                        class="w-full mb-4" />

                    <flux:button icon="plus" type="submit">Add Content</flux:button>
                </div>
            </form>
        </div>
    </div>
